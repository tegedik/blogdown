...
<iframe width="560" height="315" src="https://www.youtube.com/embed/UCmPmkHqHXk" frameborder="0" allowfullscreen></iframe>

The device demonstrates central limit theorem, specifically how binomial distribution approximates to normal distribution. As you can see in the video, there are pegs on the board arranged in a rectangular shape. You drop a single bean, the bean hits the peg and falls left or right with some probability ($p$). Since we assume that the device is constructed well (i.e., unbiased), we expect the bean goes both sides with equal probability, $p=1-p=q=0.5$. This step is repeated for each row of pegs and the bean ends up in a (corresponding, rectangular) bin. If the probability of bouncing right is $p$ (in our case, $0.5$), the number of rows is $N$, and the number of times the bean bounces to right is $n$, then the probability of the bean ending up in the $n$th bin from left is,
...
```{r nb, echo=FALSE, eval=TRUE, message=FALSE, warning=FALSE}
set.seed(1)
...
```{r np, eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE}
set.seed(7)
...
There is no way for us to know where a single bean would end up. But under certain conditions, it is possible to know the distribution of thousands of beans. This is what Galton [-@Galton1889] called "Order in Apparent Chaos" (p.66) [^1]:

> I know of scarcely anything so apt to impress the imagination as the wonderful form of cosmic order expressed by the "Law of Frequency of Error." The law would have been personified by the Greeks and deified, if they had known of it. It reigns with serenity and in complete self-effacement amidst the wildest confusion. The huger the mob, and the greater the apparent anarchy, the more perfect is its sway. It is the supreme law of Unreason. Whenever a large sample of chaotic elemts are taken in hand and marshalled in the order of their magnitude, an unsuspected and most beautiful form of regularity proves to have been latent all along.

<!-- As Michael said at the end of the video: "Thank you randomness. Thank you, even more, statistics." And thank you, Francis Galton. -->

[^1]: *Natural Inheritance* is available as PDF [here](http://galton.org/books/natural-inheritance/index.html).
